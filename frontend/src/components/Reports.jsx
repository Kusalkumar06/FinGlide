        ...
        <div className='flex justify-between w-full max-w-3xl bg-[#FFFAF4] px-3 py-2 rounded-lg gap-2'>
          {
            reportTabs.map((tab,index) => (
              <button key={index} onClick={() => dispatch(actions.setReportsTab(tab.id))} className={`${activeReportsTab === tab.id && "bg-white shadow"} py-2 px-2 sm:px-3 flex-1 rounded text-sm sm:text-base transition-all`}>{tab.display}</button>
            ))
          }
        </div>
      </div>
